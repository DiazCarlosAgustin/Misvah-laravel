<template>
    <div class="col-12 col-xs-12 col-md-12 col-lg-8 border-right">
        <form action="" @submit.prevent="updateProducto">
                ...
                <div class="table-wrapper-scroll-y my-custom-scrollbar">
                    <table class="table  mb-0">
                        <thead>
                            <tr>
                                <th
                                    class="align-center text-center align-middle"
                                >
                                    Color
                                </th>
                                <th
            ...
            <div class="form-group">
                <label for="txtInformacionProducto"
                    >Informacion adicional:</label
                >
                <textarea
                    name="txtInformacionProducto"
                    id="txtInformacionProducto"
                    class="form-control w-100"
                    cols="30"
                    rows="10"
                    v-model="producto.infomacion"
                ></textarea>
            </div>
            <div v-if="success">
...
export default {
    ...
    methods: {
        ...
        updateProducto: function() {
            ...
            axios
                .put(this.url + "producto/" + params.id, params)
                .then(res => {
                    this.error = res.data.error;
                    this.success = res.data.success;

                    if (res.data.success)
                        this.$emit("updateProductos", this.producto);
                })
                .catch(err => {
                    console.log(err);
                });
